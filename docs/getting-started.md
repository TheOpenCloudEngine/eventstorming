...
| **항 목**           | **표준 개발 방법론 적용시,**        | **이벤트스토밍 도구 적용시,**              |
| ----------------- | ----------------------------- | ---------------------- |
|  도구적 제약 | 분석,설계 담당자가 전문 도구 학습을 통해 산출물 작성       | 아무런 도구 없이 도메인 전문가에서 UI/UX 담당자, 개발자가 분석설계 수행 가능        |
| 분석, 설계 패턴    | 데이터 구조를 먼저 정의하므로, 도메인간 JOIN Query 발생 확률 높음       | 비즈니스 도메인을 먼저 식별한 후, 개별 서비스를 정의함으로 JOIN Query를 사전에 차단                  |
| MSA 코드 구현 생산성         | MSA 구현 스킬을 보유한 개발자에 의한 수작업 개발 | 도구에서 초기 소스코드 자동 생성      |
| 폴리글랏 MSA      | 프레임워크가 제공하는 단일 언어 사용으로 현실적으로 구현이 어려움                      | 사용자정의 확장 템플릿으로  서비스별 특성에 맞는 언어로 생성 가능 |
| 시스템 유지보수 측면        | SM 조직이 표준 프레임웍크를 이해해야 한다는 제약 존재  | 유비쿼터스 언어 적용 및 이벤트스토밍 스티커별 MSA Chassis 적용에 따라 상대적으로 SM이 유리        |

<p align="center"> 표 2 이벤트스토밍 도구 도입효과 </p>

이벤트스토밍 도구는 분석에서부터 운영에 이르는 MSA 전 생애주기를 지원한다. 기존 MSA 스킬을 보유한 개발자가 직접 수작업으로 개발하던 소스코드는 도구에서 초기 소스코드를 자동으로 생성해 주며, 오프라인 이벤트스토밍에서는 불가능한 결과물 버전관리와 수행 결과물의 전자적 보관은 소프트웨어 기반의 이벤트스토밍 툴인 ES2Cd가 제공하는 잇점 중 하나이다.
